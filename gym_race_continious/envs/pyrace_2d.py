...
class PyRace2D:
    ...
    def action(self, action):
        # Apply continuous acceleration, steering, and braking
        acceleration, steering, braking = action

        # Update speed based on acceleration and braking
        self.car.speed += 2 * acceleration
        self.car.speed -= 2 * braking

        # Update steering angle based on the steering action
        self.car.angle += 5 * steering

        # Clip speed to be within a range
        self.car.speed = max(1, min(self.car.speed, 10))
